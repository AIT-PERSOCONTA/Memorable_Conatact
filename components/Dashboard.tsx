...
const Dashboard: React.FC<DashboardProps> = ({ theme }) => {
  ...
  return (
    <div className={`flex flex-col lg:flex-row min-h-[600px] lg:h-[850px] w-full max-w-7xl mx-auto overflow-hidden rounded-[24px] lg:rounded-[40px] border transition-all duration-500 ${theme === 'dark' ? 'bg-slate-950 border-slate-800 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.7)]' : 'bg-white border-slate-200 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.08)]'}`}>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* Header / Search */}
        <header className={`px-6 lg:px-12 py-5 lg:py-7 border-b flex items-center justify-between gap-4 md:gap-12 transition-colors duration-300 ${theme === 'dark' ? 'border-slate-800 bg-slate-950/50' : 'border-slate-200 bg-white'}`}>
          <div className="flex lg:hidden items-center gap-4">
            <button onClick={() => setIsSidebarOpen(true)} className={`p-2.5 rounded-xl border ${theme === 'dark' ? 'border-slate-800 bg-slate-900 text-slate-400' : 'border-slate-200 bg-slate-50 text-slate-500'}`}>
              <Menu className="w-5 h-5" />
            </button>
          </div>

          <div className="relative flex-1 max-w-2xl">
            <Search className="absolute left-4 lg:left-6 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
            <input
              type="text"
              placeholder="Query network..."
              className={`w-full py-3 lg:py-4 pl-12 lg:pl-14 pr-4 rounded-xl lg:rounded-2xl text-xs lg:text-sm border font-black focus:outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all ${theme === 'dark' ? 'bg-slate-900 border-slate-700 text-slate-200 placeholder:text-slate-600' : 'bg-white border-slate-200 text-slate-900 shadow-sm placeholder:text-slate-400'
                }`}
            />
          </div>
          <div className="flex items-center gap-3 lg:gap-6">
            <button className={`p-3 lg:p-4 rounded-xl lg:rounded-2xl border relative ${theme === 'dark' ? 'bg-slate-900 border-slate-700 text-slate-400' : 'bg-white border-slate-200 text-slate-500'}`}>
              <Bell className="w-4 h-4 md:w-5 md:h-5" />
              <span className="absolute top-3 right-3 w-2.5 h-2.5 bg-indigo-500 rounded-full border-2 border-white dark:border-slate-950 shadow-sm" />
            </button>
            <button
              onClick={() => setIsContactModalOpen(true)}
              className="hidden sm:flex items-center gap-2 px-5 lg:px-8 py-3 lg:py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl lg:rounded-2xl text-xs lg:text-sm font-black shadow-2xl shadow-indigo-600/30 transition-all uppercase tracking-widest whitespace-nowrap"
            >
              ...
              >
                ...
                <div className={`overflow-x-auto rounded-[24px] lg:rounded-[32px] border transition-colors duration-300 ${theme === 'dark' ? 'border-slate-800 bg-slate-900/40' : 'border-slate-200 bg-white shadow-sm'}`}>
                  <table className="w-full text-left border-collapse min-w-[700px]">
                    <thead>
                      <tr className={`border-b text-[9px] lg:text-[10px] font-black uppercase tracking-[0.25em] text-slate-500 transition-colors duration-300 ${theme === 'dark' ? 'border-slate-800' : 'border-slate-100 bg-slate-50/80'}`}>
                        <th className="px-6 lg:px-10 py-5 lg:py-6 text-left">Connection</th>
                        <th className="px-6 lg:px-10 py-5 lg:py-6 text-left">Role & Entity</th>
                        <th className="px-6 lg:px-10 py-5 lg:py-6 text-left">Capture Origin</th>
                        <th className="px-6 lg:px-10 py-5 lg:py-6 text-left">Coordinates</th>
                        <th className="px-6 lg:px-10 py-5 lg:py-6"></th>
                      </tr>
                    </thead>
                    <motion.tbody
                        ...
                        >
                          <td className="px-6 lg:px-10 py-5 lg:py-6">
                            <div className="flex items-center gap-3 lg:gap-5">
                              <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-xl lg:rounded-2xl overflow-hidden shadow-xl`}>
                                {contact.image ? (
                                  <img src={contact.image} alt={contact.name} className="w-full h-full object-cover" />
                                ) : (
                                  <div className={`w-full h-full ${contact.color} flex items-center justify-center text-xs lg:text-sm font-black text-white`}>
                                    {contact.initials}
                                  </div>
                                )}
                              </div>
                              <div className="flex flex-col text-left">
                                <span className={`text-xs lg:text-sm font-black ${theme === 'dark' ? 'text-slate-200' : 'text-slate-900'}`}>{contact.name}</span>
                                <span className="text-[9px] font-bold text-slate-500">{contact.phone}</span>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 lg:px-10 py-5 lg:py-6">
                            <div className="flex flex-col text-left">
                              <span className={`text-xs lg:text-sm font-black ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}>{contact.title}</span>
                              <span className="text-[9px] lg:text-[10px] font-bold text-slate-500 uppercase tracking-widest">at {contact.company}</span>
                            </div>
                          </td>
                          <td className="px-6 lg:px-10 py-5 lg:py-6">
                            <span className={`px-3 lg:px-4 py-1.5 lg:py-2 rounded-lg lg:rounded-xl text-[9px] lg:text-[10px] font-black uppercase tracking-widest whitespace-nowrap ${theme === 'dark' ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-600'}`}>
                              {contact.event}
                            </span>
                          </td>
                          <td className="px-6 lg:px-10 py-5 lg:py-6">
                          ...
                          <td className="px-6 lg:px-10 py-5 lg:py-6 text-right">
                            <button className="p-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity rounded-xl hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500">
