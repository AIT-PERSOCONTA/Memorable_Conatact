    ...
    <HelmetProvider>
      <div className={`relative min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-950 text-slate-50' : 'bg-slate-50 text-slate-900'}`}>
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          {view === 'about' ? (
            <>
              <SEO title="About Us" description="Learn about the mission behind MemorableContact and how we're revolutionizing professional networking." />
              <About theme={theme} onBack={() => setView('home')} />
            </>
          ) : view === 'resources' ? (
            <>
              <SEO title="Resources & Guides" description="Deep dives into AI networking, memory science, and professional relationship management." />
              <Resources theme={theme} onBack={() => setView('home')} />
            </>
          ) : view === 'pricing' ? (
            <>
              <SEO title="Premium Plans" description="Choose the perfect plan to scale your professional memory. Monthly and annual options available." />
